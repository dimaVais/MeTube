...
const API_KEY = 'AIzaSyDGql0MyVMEQeH89LQj0TtpM66SoLpkAhw';
...
async function loadSongs(search) {
  try {
    const queryStrKey = `?&part=snippet&maxResults=25&videoEmbeddable=true&type=video&key=${API_KEY}&q=${search}`;
    const res = await axios.get(`${YOUTUBE_URL}${queryStrKey}`);
    console.log(res);
